...
function nextDateClass(r) {
  if (!r.next_date) return 'text-mushroom-400'
  const today = new Date().toISOString().slice(0, 10)
  if (r.next_date <= today) return 'text-tomato-600 font-medium'
  return 'text-mushroom-400'
}
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-lg font-medium text-mushroom-950">{{ viewLabel }} Recurring</h2>
        <p class="text-xs text-mushroom-400 mt-0.5">Automated expense templates</p>
      </div>
    ...
    <transition name="fade">
      <div v-if="showForm" class="card-elevated p-5 space-y-4">
        <h3 class="text-sm font-medium text-mushroom-950">{{ editingRule ? 'Edit Rule' : 'New Recurring Rule' }}</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            <label class="label-text">End Date <span class="text-mushroom-300">(optional)</span></label>
        ...
        <div class="flex items-center gap-2">
          ...
          <button @click="cancelForm" class="text-xs text-mushroom-400 hover:text-mushroom-600">Cancel</button>
        </div>
      </div>
    </transition>
    ...
    <div v-if="loading && !rules.length" class="space-y-3">
      <div v-for="r in 3" :key="r" class="card-elevated p-4 border-l-4 border-l-mushroom-200">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
    ...
    <div v-else-if="!rules.length" class="text-center py-12">
      ...
      <p class="text-sm text-mushroom-500">No recurring rules yet</p>
      <p class="text-xs text-mushroom-400 mt-1">Click "Add Rule" to set up automated expenses</p>
    </div>

    <div v-else class="space-y-3">
      <div v-for="r in rules" :key="r.id" class="card-elevated p-4 border-l-4" :class="r.active ? 'border-l-kangkong-500' : 'border-l-mushroom-200'">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <button
              @click="handleToggle(r)"
              class="relative w-9 h-5 rounded-full transition-colors"
              :class="r.active ? 'bg-kangkong-500' : 'bg-mushroom-200'"
            ...
            <div>
              <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-medium text-mushroom-950">{{ r.name }}</span>
                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium" :class="r.active ? 'bg-kangkong-100 text-kangkong-700' : 'bg-mushroom-100 text-mushroom-400'">{{ r.active ? 'Active' : 'Paused' }}</span>
              </div>
              <div class="flex items-center gap-3 mt-1 text-xs text-mushroom-400">
                <span>{{ frequencyLabels[r.frequency] }} on {{ r.day_of_month }}{{ r.day_of_month === 1 ? 'st' : r.day_of_month === 2 ? 'nd' : r.day_of_month === 3 ? 'rd' : 'th' }}</span>
                <span class="text-mushroom-200">|</span>
                <span class="font-medium text-mushroom-700">{{ currencySymbol }}{{ r.amount.toLocaleString(undefined, { minimumFractionDigits: 2 }) }}</span>
                <span class="text-mushroom-200">|</span>
          ...
          <div class="flex items-center gap-2">
            <button @click="startEdit(r)" class="text-mushroom-300 hover:text-blueberry-500 transition-colors" title="Edit">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </button>
            <button @click="handleDelete(r)" class="text-mushroom-300 hover:text-tomato-500 transition-colors" title="Delete">
