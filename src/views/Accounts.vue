...
const accountTypeColors = {
  ...
  investment: 'border-l-carrot-600',
}
  ...
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <button @click="eyeHidden = !eyeHidden" class="text-mushroom-600 dark:text-mushroom-300 hover:text-mushroom-600 dark:hover:text-mushroom-300">
    ...
    <div v-else class="space-y-6">
      <div v-for="(group, gi) in groupedByType" :key="group.type">
        <h3 class="text-xs font-semibold uppercase tracking-wider text-mushroom-600 dark:text-mushroom-300 mb-3 mt-4 first:mt-0">{{ group.label }}</h3>
            ...
            <div class="flex items-center justify-between mb-2">
              <div>
                <div class="flex items-center gap-2.5 mb-1">
                  <span v-if="acc.bank" class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium" :class="bankColors[acc.bank] || 'bg-mushroom-100 dark:bg-mushroom-800 text-mushroom-700 dark:text-mushroom-300'">{{ acc.bank }}</span>
                  <input
                ...
                <template v-if="editingAcctNum === acc.id">
                  <template v-if="confirmingAcctNum === acc.id">
                    <div class="flex items-center gap-1.5 mt-1">
                      <span class="text-xs text-mushroom-600 dark:text-mushroom-300">Save change?</span>
                      <button @click="saveAcctNum(acc)" class="text-kangkong-600 hover:text-kangkong-800 text-xs font-medium">Yes</button>
                      <button @click="cancelAcctNum" class="text-mushroom-600 dark:text-mushroom-300 hover:text-mushroom-600 dark:hover:text-mushroom-300 text-xs">No</button>
                    </div>
                  </template>
                  <template v-else>
                ...
                <div v-else-if="acc.account_number" class="text-xs text-mushroom-600 dark:text-mushroom-300 mt-1 cursor-pointer hover:text-kangkong-600 dark:hover:text-kangkong-400" @click="startEditAcctNum(acc)">{{ acc.account_number }}</div>
                <div v-else class="text-xs text-mushroom-500 dark:text-mushroom-500 mt-1 cursor-pointer hover:text-kangkong-600 dark:hover:text-kangkong-400" @click="startEditAcctNum(acc)">+ Add account number</div>
                <div v-if="acc.dividend_type" class="text-xs text-mushroom-600 dark:text-mushroom-300 mt-1">{{ acc.dividend_type }}</div>
                <template v-if="acc.type === 'time_deposit'">
                  ...
                  <template v-else>
                    <div class="text-xs text-mushroom-600 dark:text-mushroom-300 cursor-pointer hover:text-kangkong-600 dark:hover:text-kangkong-400 mt-2 pt-2 border-t border-mushroom-100 dark:border-mushroom-700/50" @click="startEditMaturity(acc)">
                      {{ acc.maturity_date ? 'Maturity: ' + acc.maturity_date : '+ Set maturity date' }}
                    </div>
                  </template>
                </template>
                <div v-else-if="acc.maturity_date" class="text-xs text-mushroom-600 dark:text-mushroom-300 mt-2 pt-2 border-t border-mushroom-100 dark:border-mushroom-700/50">Maturity: {{ acc.maturity_date }}</div>
              </div>
              <div class="text-right">
                ...
                <div class="flex justify-end mt-1">
                  ...
                  <button v-else @click="confirmingDelete = acc.id" class="text-mushroom-500 dark:text-mushroom-500 hover:text-tomato-500 dark:hover:text-tomato-400 transition-colors" title="Delete account">
                ...
                <template v-if="editingGoal === acc.id">
                  <div class="flex items-center gap-2">
                    <input
                      ...
                      autofocus
                    />
                    <span class="text-xs text-mushroom-600 dark:text-mushroom-300">Press Enter to save</span>
                  </div>
                </template>
                <template v-else>
                  <BudgetProgressBar
                    ...
                    class="mb-2"
                  />
                  <div v-if="acc.goal_amount > 0" class="flex items-center justify-between text-xs text-mushroom-700 dark:text-mushroom-400">
