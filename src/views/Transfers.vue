...
<template>
  <div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-lg font-medium text-mushroom-950">{{ viewLabel }} Transfers</h2>
        <p class="text-xs text-mushroom-400 mt-0.5">Move money between accounts</p>
      </div>
    ...
    <transition name="fade">
      <div v-if="showForm" class="card-elevated p-5 space-y-4">
        <h3 class="text-sm font-medium text-mushroom-950">New Transfer</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            <label class="label-text">Fee ({{ currencySymbol }}) <span class="text-mushroom-300">optional</span></label>
          ...
          <div>
            <label class="label-text">Note <span class="text-mushroom-300">optional</span></label>
    ...
    <div v-if="loading && !transfers.length" class="space-y-3">
      <div v-for="t in 3" :key="t" class="card-elevated p-4 border-l-4 border-l-mushroom-200">
        <div class="flex items-center justify-between">
          <div>
    ...
    <div v-else-if="!transfers.length" class="text-center py-12">
      ...
      <p class="text-sm text-mushroom-500">No transfers yet</p>
      <p class="text-xs text-mushroom-400 mt-1">Click "New Transfer" to move money between accounts</p>
    </div>
    ...
    <div v-else class="space-y-3">
      <div v-for="t in transfers" :key="t.id" class="card-elevated p-4 border-l-4 border-l-blueberry-400">
        <div class="flex items-center justify-between">
          <div>
            <div class="flex items-center gap-2 mb-1">
              <span class="text-sm font-medium text-mushroom-950">{{ accountName(t.from_account_id) }}</span>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-mushroom-300"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              <span class="text-sm font-medium text-mushroom-950">{{ accountName(t.to_account_id) }}</span>
            </div>
            <div class="flex items-center gap-3 text-xs text-mushroom-400">
              <span>{{ formatDate(t.date) }}</span>
              <template v-if="t.fee > 0">
                <span class="text-mushroom-200">|</span>
                <span class="text-tomato-500">Fee: {{ currencySymbol }}{{ t.fee.toLocaleString(undefined, { minimumFractionDigits: 2 }) }}</span>
              </template>
          ...
          <div class="flex items-center gap-3">
            <span class="text-lg font-medium text-mushroom-950">{{ currencySymbol }}{{ t.amount.toLocaleString(undefined, { minimumFractionDigits: 2 }) }}</span>
            <div class="flex items-center gap-1">
              <template v-if="confirmingDelete === t.id">
                <span class="text-xs text-mushroom-400 mr-1">Reverse?</span>
              ...
              <button v-else @click="confirmingDelete = t.id" class="text-mushroom-300 hover:text-tomato-500 transition-colors" title="Reverse transfer">
